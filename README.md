...
```sql
SELECT
  ROUND(AVG("이용시간"), 2) AS avg_minutes
FROM "서울특별시_공공자전거_대여이력_정보"
WHERE "이용시간" IS NOT NULL;
```

이용 평균 시간을 통해 따릉이의 대표적인 이동 길이를 확인하였다. 평균 이용시간은 자전거 회전율과 운영 효율에 영향을 주는 핵심 지표로 활용할 수 있다.

- **핵심 인사이트**: 평균 이용시간이 낮게 나타난다면 단거리 이동 중심의 생활형 이용이 우세하다고 해석할 수 있어 출퇴근 피크 시간대 재배치에 집중하는 전략이 효과적이다. 반대로 평균 이용시간이 높다면 여가성 이용 비중이 큰 것으로 판단되어 주말·관광지 중심의 공급 전략이 유리하다.
