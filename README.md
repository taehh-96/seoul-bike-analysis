...
- **정량 요약**: 순유출 상위 10개 합계 -20,595건(전체 순유출의 5.62%), 순유입 상위 10개 합계 +22,254건(전체 순유입의 6.94%)
- **비교 기준**: 상위 부족 10개 평균 순유출 2,059.50건으로 전체 대여소 평균 |순유출| 246.13건 대비 8.37배
- **피크 시간대 순유출**: 상위 부족 10개 기준 07–09시/15–16시 합계 -8,780건(상위 부족 10개 순유출의 42.63%)
...
5. 재배치 우선순위 기준: 순유출 규모 상위, 피크 시간대 순유출 비중, 대여량 상위 거점을 우선 대응한다.

## ⚠️ 분석 한계
- 3개월 데이터만 사용하여 장기적인 계절성을 반영하지 못함
- 날씨, 강수량, 공휴일·행사 등의 외부 요인을 고려하지 못함
...
## ♻️ 재현 방법
- SQL 쿼리 모음: `sql/analysis_queries.sql`
- CSV 집계 스크립트: `python scripts/compute_station_metrics.py --data-dir "<CSV 폴더 경로>"`
